...
## Phase 4B Secure MCP Tunnel supervision and system tray addendum

Phase 4B lets the Console supervise a configured Secure MCP Tunnel profile. Tunnel profile creation remains external: the Console does not run `tunnel-client init`, edit profiles, configure connectors, or accept Tunnel IDs, API keys, OAuth data, or other identity input. `CODEX_BRIDGE_TUNNEL_EXECUTABLE` is an optional explicit executable override and `CODEX_BRIDGE_TUNNEL_PROFILE` defaults to `codex-bridge`; the profile is accepted only when it contains 1–64 ASCII letters, digits, `.`, `_`, or `-`. Executable priority is explicit override, PATH `tunnel-client.exe`, then PATH `tunnel-client`; an invalid explicit override fails closed, `.ps1` is excluded, and Windows candidate deduplication uses canonical case-insensitive paths.

After both Bridge and App Server readiness, the Console runs one asynchronous direct-process doctor preflight with `doctor --profile <profile> --explain --health.listen-addr 127.0.0.1:0`. The child inherits the current environment; no environment values are read for display, logging, or persistence. Doctor has a ten-second timeout and an 8 KiB combined stdout/stderr bound, and raw doctor output is discarded. A successful check enables Tunnel start.

The Console-owned Tunnel is a managed `QProcess`, never detached. Immediately before launch, it obtains a process-local ephemeral port by binding `127.0.0.1:0` through `QTcpServer`, then starts `run --profile <profile> --health.listen-addr 127.0.0.1:<port>` without a shell or newly constructed private target URL. The supervisor exposes `unavailable`, `checking`, `ready_to_start`, `starting`, `running`, `ready`, `not_ready`, `stopping`, `stopped`, and `failed`. It polls only the loopback `/healthz` and `/readyz` endpoints asynchronously, keeps no response body, marks a live process `not_ready` after the bounded startup window, and continues low-frequency polling. Unexpected exit is `Tunnel: failed`; automatic Tunnel restart is disabled. Stop is `terminate`, a QTimer-bounded two-second fallback to `kill`, and applies only to the Console-owned process. external Tunnel is never discovered/taken over.

The Console adds `Start Tunnel`, `Stop Tunnel`, and `Restart Tunnel` controls plus standard PySide6 `QSystemTrayIcon`, `QMenu`, and `QAction` entries for `Show Console`, `Hide Console`, the same Tunnel controls, and `Exit`. The window close minimizes/hides to tray when available; it does not stop Bridge or the Console-owned Tunnel. When the tray is unavailable, close follows explicit cleanup. explicit Exit stops Console-owned Tunnel after Console timers, SSE/HTTP replies, and probes are cleaned up, then quits; Bridge remains running on Console Exit. Tunnel secrets/identity are not stored by CodexBridge. Bridge Stop/Restart is Phase 4C.

## Phase 4C authenticated localhost Bridge control and graceful Stop/Restart

Phase 4C adds Bridge lifecycle controls to the existing Console and tray without changing the three-pane layout or the nine-tool MCP interface. Only a Bridge started by the current Console session is controllable. A Bridge already ready when the Console starts is `Runtime: external`; external Bridge is never taken over and `Start Bridge`, `Stop Bridge`, and `Restart Bridge` remain disabled. Console Exit still performs only Console cleanup and Console-owned Tunnel stop; it does not stop Bridge.

Each detached launch generates a fresh `secrets.token_urlsafe(32)` control token. The token is held only in process-local Console memory and is passed only through the inherited `CODEX_BRIDGE_CONTROL_TOKEN` environment override, alongside the existing `CODEX_BRIDGE_CODEX_EXECUTABLE` and `CODEX_BRIDGE_UI_PORT` overrides. It is never persisted, placed in settings, shown in the GUI/status/activity, logged, included in stdout/stderr/errors, put in a URL or request body, or sent to the Tunnel. `BridgeConfig` treats an unset/empty token as `None` and accepts only 32–256 ASCII URL-safe characters without echoing token values in configuration errors.

The UI application registers `POST /ui-api/control/shutdown` only when both a valid configured token and a shutdown callback are present. The route exists only on the fixed `127.0.0.1:<ui_port>` UI listener; it is not mounted on MCP or exposed through the Tunnel. It accepts only `Authorization: Bearer <token>`, compares the candidate with `hmac.compare_digest`, returns fixed `403` for missing/malformed/wrong credentials, and returns fixed `202` JSON `{"status":"shutdown_requested"}` for valid credentials. The callback is a Starlette `BackgroundTask`, so the response is sent before the outer Uvicorn controller sets `server.should_exit = True`.

`run_server()` binds that controller to the outer MCP Uvicorn server. The normal chain is control request → outer Uvicorn graceful exit → Starlette lifespan `finally` → existing `BridgeRuntime.shutdown()`. The runtime retains its existing order: interrupt active turns, shut down the UI listener, then shut down the Codex App Server. Stopping or restarting Bridge may interrupt active Codex turns. The Console does not implement a second interrupt or direct App Server shutdown path.

The Console uses one bounded Qt Network POST client. Stop and Restart are enabled only when Bridge and App Server are ready, the runtime is `started by Console`, the current token is present, and neither Bridge nor Tunnel is in a lifecycle transition. If a Console-owned Tunnel is `running`, `ready`, or `not_ready`, Stop/Restart waits for Tunnel stop completion before issuing the Bridge control POST. After `202`, Qt timers poll `/healthz` and `/ui-api/status` without blocking the GUI; only endpoint unreachability confirms `Runtime: stopped`. Confirmed stop clears the old token, PID, and detached-launch guard, allowing a new Start Bridge. A control failure or roughly ten-second timeout remains fail closed and performs no retry or process kill.

Restart remembers whether a Console-owned Tunnel was managed and running, performs graceful stop and disappearance confirmation, generates a fresh token, launches the detached Bridge, waits for readiness, and starts the Tunnel exactly once only if it was running before Restart. If relaunch or readiness fails, Tunnel is not automatically started. The window and tray actions use the same lifecycle-state calculation. No OS process enumeration, PID takeover, taskkill, signal, force-kill fallback, persistent token, remote control route, automatic crash restart, or MCP shutdown/restart tool is added. The MCP interface remains exactly nine tools and no dependency is added.
